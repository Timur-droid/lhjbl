...
public:
	House(int floor = 1, string Type = "House", double money = 0) : floor(floor), Type(Type), money(money) {
		...
		for (auto acc : accounts1) {
			fs.write((char*) &accounts1, sizeof(Account));
		}
		fs.close();
		fs.open("Accounts.txt");
		for (auto acc : accounts1) {
	...
	void Arenda(int acc_no, int pin, double amount, double month) {
		...
		if (find == 1) {
			cout << "Sellers telephone number - +9989********" << endl;
			cout << "If you arended this house, you will have " << getMoney(1) << "$ !" << endl;
		}
	}
...
vector<Account*> House::accounts;
vector<Account*> House::accounts1;